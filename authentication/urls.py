...
urlpatterns = [
    ...
    url(r'^change-password/$', auth_views.PasswordChangeView.as_view(template_name="authentication/change_password.html", success_url="/authentication/change-password-done"), name='password_change'),
    ...
    url(r'^reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', auth_views.PasswordResetConfirmView.as_view(template_name="authentication/set_password.html", success_url="/authentication/reset-complete-done/"), name='password_reset_confirm'),
